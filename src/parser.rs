...
peg::parser! {
    pub grammar rlang_parser<'a>(toks: &'a Toks<'a>) for Toks<'a> {
        ...
        pub rule qual() -> Qualification
            ...
            / ty:stype() tok(DoubleEquals) ty2:stype() { Qualification::Equiv(ty, ty2) }
        ...
        pub rule quant() -> Quantification
            = tok(BracketL) ids:sid()+ tok(Colon) kind:skind() tok(BracketR) qs:qual_clause()?
                {
                    let bindings = ids.into_iter().map(|id| (id, kind.clone())).collect();
        ...
        pub rule expr_lam() -> Expr
            ...
            / tok(Let) x:sid() tok(Equals) e1:sexpr_ann() tok(In) e2:sexpr_lam()
              { Expr::Let(x, Box::new(e1), Box::new(e2), None) }
            / tok(Let) r:tok(Rec)? x:sid() q:squant()? tok(Colon) t:stype() c:sclause() tok(In) e:sexpr_lam()
              { Expr::LetDecl(x, t, q, Box::new(c), Box::new(e), r.is_some()) }
